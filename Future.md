...
Retrieve: Call findSimilarContent(query) to get relevant chunks.

Augment: Combine the chunks into a single context string.

Generate: Create a prompt (system message + context + user query) and send it to the ChatLanguageModel.
...
Retrieve: Fetch a larger number of chunks (e.g., top 10) from Infinispan.

Re-Rank: (Optional, very advanced) Use a more powerful model (like a cross-encoder) to re-rank these 10 chunks based on their relevance to the specific query.

Augment: Use the new top 3-5 re-ranked chunks for the prompt.
